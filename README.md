...
Here are a few examples of good and poor object detection/classification:

![Messed Up](images/messed_up_hoop.png)
![Corrected](images/corrected.png)

The messed up image is likely a result of the camera moving. However, when you slow down the video and lower the frame rate, we can see that the CSRT tracking does a rather good job of detecting basketball shots. The real inaccurracies here come from live video detection, where the basketbal moves at an alarming rate that overwhelms my current hardware setup.
